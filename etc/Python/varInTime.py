...
for j in range(0, sim, interval):
    plt.loglog(t[j], var[j], ls="%s" % lin[j], lw=5, color="%s" % col[j], label="variance", marker="s", markersize=10)
...
# plt.savefig(os.path.join(latexFolderPath, "images/mediumPeViT.png"))
...
for j in range(0, sim, interval):
    ...
    plt.loglog(t[j], ivar[j], ls="%s" % lin[j], lw=5, color="%s" % col[j], label='inertial variance', marker="s", markersize=10)
...
plt.axis([min(min(t, key=min)), 2*10**4, min(min(ivar, key=min)), max(max(ivar, key=max))])
plt.legend(loc="best", fontsize=30)
# plt.savefig(os.path.join(latexFolderPath, "images/mediumPeIViT.png"))
