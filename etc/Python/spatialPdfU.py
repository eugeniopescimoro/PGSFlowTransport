...
# 0) Set the path for the bash script to be executed, usually the testcase home folder
    ...
    # homeFolder = Path(os.path.join('/data/pmxep5-8/PGSFlowTransport/tutorials/RESULTS/Herten/Herten7_Stochastic/', simPath[i]))
    homeFolder = Path('/data/pmxep5-8/PGSFlowTransport/tutorials/RESULTS/Herten/realismDegree/Herten8_Ephesia')
    latexFolderPath = Path('/home/pmxep5/Git/Overleaf/Thesis')

# # 1) Verify the following functionObject to be present in system/fieldMetricsDict
# functions
# {
#     magScaled
#     {
...
# 3a) Plot spatial pdf with Python
    ...
    # functionObject and spatialPdf yield the PDF of the velocity spatial field
    subprocess.run(['/bin/bash', '-c', 'mpirun --oversubscribe -np 12 postProcess -func magScaled -dict system/fieldMetricsDict -field U -time 0 -parallel']) # It runs the functionObject added in 1) and the output is stored in processor*/0/magUscaled
    subprocess.run(['/bin/bash', '-c', 'mpirun --oversubscribe -np 12 spatialPdf -parallel -field magUscaled -time 0 -logbin -nbins 50']) # It runs the spatialPdf post processing utility and stores the output in postProcessing/png/0/magUscaled-none_
    col = ['0.0', '0.2', '0.4', '0.6']
    lab = ['0.4', '0.6', '0.8', '1.0']
    minYaxis = 1e-3
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(latexFolderPath, "images/magUscaledHerten8.png"))
